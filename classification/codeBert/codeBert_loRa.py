...
# LoRA — install with: pip install peft
...
#    → only the Q and V projection matrices in every attention layer
#      receive LoRA adapters. K and the FFN remain fully frozen.
#      This is the standard choice: Q and V control what is attended
#      to and what information is retrieved; K only computes keys and
#      is less sensitive to fine-tuning.
#
#  r=16
#    → rank of the two low-rank matrices A (d×r) and B (r×d).
#      Higher r = more capacity but more params.
#      r=16 is a safe default for a classification task.
#
#  lora_alpha=32
#    → scaling factor applied to the LoRA output: (alpha/r) * B*A*x
#      Setting alpha = 2×r keeps the effective scale at 2.0 regardless
#      of r, making it easy to compare runs with different ranks.
#
#  lora_dropout=0.1
#    → dropout applied inside the LoRA branch for regularisation.
#      Especially useful here because the dataset is small (581 samples).
#
#  bias="none"
#    → do not train any bias terms. Biases contribute little but add
#      noise when fine-tuning on very small datasets.
#
...
# ======================================================
# Early Stopping — Macro-F1 based (higher = better)
# ======================================================
class EarlyStopping:
    ...
    def step(self, score, model):
        if self.best_score is None or score > self.best_score + self.min_delta:
            ...
            # Save only the trainable LoRA + classifier params (much smaller than full model)
            self.best_state = {
                k: v.cpu().clone()
...
def main_code_bert():
    ...
    # Single .astype("category") call — guarantees label_names[i] ↔ code i
    category_col = df["Category"].astype("category")
    labels       = category_col.cat.codes.values
    label_names  = category_col.cat.categories.tolist()
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(comments, labels), 1):
        ...
        fold_weights  = compute_class_weight("balanced", classes=fold_classes, y=labels[train_idx])
        ...
        # Model + LoRA
        #
        #  Step 1: load the full pre-trained CodeBERT with a fresh
        #          classification head (num_labels logits).
        #
        #  Step 2: wrap it with get_peft_model(model, LORA_CONFIG).
        #          This injects LoRA adapters into every "query" and
        #          "value" matrix of the 12 attention layers, then
        #          freezes ALL original weights automatically.
        #
        #  Why this helps on 581 samples:
        #    - Massively reduces overfitting risk
        #    - Faster training (fewer backward-pass computations)
        #    - Pre-trained code representations stay intact
        # --------------------------------------------------
        ...
        # Print trainable parameter count once (first fold only)
        if fold == 1:
            model.print_trainable_parameters()

        # --------------------------------------------------
        # Optimizer — single LR for all trainable params
        #
        #  With LoRA, the encoder is frozen so differential LRs
        #  are no longer needed. A higher LR (3e-4) is appropriate
        #  because only the small LoRA matrices + classifier are updated.
        #  They start from near-zero (A init: random Gaussian, B init: 0)
        #  so they need a larger step to converge quickly.
        # --------------------------------------------------
        optimizer = AdamW(
        ...
        early_stopping = EarlyStopping(patience=4, min_delta=1e-3)
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                loss.backward()
                # Clip only trainable (LoRA) params — frozen params have no grad
            ...
            # --------------------------------------------------
            # Validation — Macro-F1 on test fold 
            # --------------------------------------------------
            model.eval()
            val_true, val_pred = [], []
            ...
            if early_stopping.step(val_macro_f1, model):
                print(f"  → Early stopping at epoch {epoch+1} "
                      f"(best Macro-F1: {early_stopping.best_score:.4f})")
                break
        ...
        # --------------------------------------------------
        # Final predictions on test fold
        # --------------------------------------------------
    ...
    # ======================================================
    with pd.ExcelWriter(
        "./results/codeBert/codebert_lora_results.xlsx",
        engine="xlsxwriter",
    ) as writer:
    ...
    print("\n✅ ALL results saved to ./results/codeBert/codebert_lora_results.xlsx")
